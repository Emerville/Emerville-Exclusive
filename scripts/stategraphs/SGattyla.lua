...
local states=
{
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.AnimState:PlayAnimation("rabbit_eat_loop", true)
        ...
        ontimeout= function(inst)
			...
			guano.Transform:SetPosition(inst.Transform:GetWorldPosition())
    ...
    State{
        ...
        onenter = function(inst) 
            local play_scream = nil
			
            if inst.components.inventoryitem then
                play_scream = inst.components.inventoryitem.owner == nil
            end
            if play_scream and math.random() > .5 then
    ...
    State{
        ...
        onenter = function(inst, data)
			...
			if inst.isonground == true and inst.isdyingbecauseisaidso == nil then
    ...
    State{
        ...
        onenter = function(inst) 
            inst.Physics:Stop()
            inst.AnimState:PlayAnimation("stunned_loop", true)
            inst.sg:SetTimeout(GetRandomWithVariance(0.6, 0.4) )
            if inst.components.inventoryitem then
        ...
        onexit = function(inst)
            if inst.components.inventoryitem then
                inst.components.inventoryitem.canbepickedup = false -- he can be still picked up // overwritten by new action
            end
        end,
...
return StateGraph("attyla", states, events, "idle", actionhandlers)
