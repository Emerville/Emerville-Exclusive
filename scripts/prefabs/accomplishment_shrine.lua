...
local function OnActivate(inst, doer)
	...
	if inst.clicks < total_clicks then
		...
		if inst.clicks % clicks_per_step == 0 then
            doer.AnimState:PlayAnimation("research")
			doer.components.talker:Say("I DID IT!")
			inst.AnimState:PlayAnimation("reward")
			SpawnPrefab("red_multifirework").Transform:SetPosition(inst.Transform:GetWorldPosition())
