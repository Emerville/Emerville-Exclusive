...
local function Greetings(inst)
    inst.components.talker:Say("I'd really love a veggie pizza right about now.")
    inst.SoundEmitter:PlaySound("dontstarve/characters/wurt/emote")	
...
local function say(inst)
    if inst ~= nil and inst.components.talker ~= nil then
        inst.components.talker:Say("The flavors are melting on my tongue!")
...
--[[   local objects = {"swap_slingshot"}	
    ...
    inst.AnimState:SetBank("wilson")
    inst.AnimState:SetBuild("wurt_peruse")
...
return Prefab("pizzawurt", fn, assets)