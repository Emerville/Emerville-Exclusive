...
--[[local function purplecane_fxanim(inst)
        inst._fx.AnimState:PlayAnimation("hit")
        inst._fx.AnimState:PushAnimation("idle_loop")
    end

    local function purplecane_oncooldown(inst)
        inst._task = nil
    end

    local function purplecane_unproc(inst)
        if inst:HasTag("forcefield") then
            ...
            inst:RemoveEventCallback("armordamaged", purplecane_fxanim)
            ...
            inst._task = inst:DoTaskInTime(TUNING.ARMOR_RUINSHAT_COOLDOWN, purplecane_oncooldown)
        end
    end

local function purplecane_proc(inst, owner)
        ...
        inst:ListenForEvent("armordamaged", purplecane_fxanim)
        ...
        inst._task = inst:DoTaskInTime(TUNING.ARMOR_RUINSHAT_DURATION, purplecane_unproc)
    end	]]
