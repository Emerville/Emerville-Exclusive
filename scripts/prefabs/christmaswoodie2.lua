...
{
   elfscarf = { "tokenappreciation1", "tokenappreciation2", "tokenappreciation3",},
}
...
local function DoLocalOffering(inst, doer)
	if inst.components.container:IsFull() then
		...
		if rewarditem then
			...
			inst.components.talker:Say("Great job helping the people of Emerville!")
...
local function Greetings(inst)
	if math.random() < chance then
        inst.components.talker:Say("Help out the people of Emerville around the world and then bring me their tokens of appreciation!", 10)
	    inst.SoundEmitter:PlaySound("dontstarve/characters/woodie/emote")
...
local function fn(Sim)
    ...
    inst.components.container:WidgetSetup("christmaswoodie2")
...
return Prefab("christmaswoodie2", fn, assets)