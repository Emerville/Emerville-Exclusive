...
local function Greetings(inst)
    ...
    inst.SoundEmitter:PlaySound("dontstarve/characters/wathgrithr/emote")	
    inst.AnimState:PlayAnimation("emoteXL_annoyed")
    inst.AnimState:PushAnimation("idle_loop")	
end

local function ontradeforgold(inst, item) 
	local nug = SpawnPrefab("goldcoin")
...
local function OnGetItemFromPlayer(inst, giver, item)
    ...
    if item.prefab == "efc" then
        inst:DoTaskInTime(20/30, ontradeforgold, item)
		inst.components.talker:Say("My love for pizza is almost the same as my love for meat!")
	end
    inst.AnimState:PushAnimation("idle_loop")		
end	   
